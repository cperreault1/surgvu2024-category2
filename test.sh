...
docker run --rm \
        ...
        -v $SCRIPTPATH/test/:/input/ \
        -v $SCRIPTPATH/:/tmp/ \
        python:3.9-slim python -c "import json, sys; f1 = json.load(open('/output/step_classification.json')); f2 = json.load(open('/tmp/expected_output_detection.json')); sys.exit(f1 != f2);"
